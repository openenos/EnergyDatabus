  ...
  def perform(panel_id) 
   ...
   #db = CassandraCQL::Database.new('127.0.0.1:9160', {:keyspace => "enos_hgv"})
    ...
    Circuit.where(:panel_id => panel_id).each do|circuit|
      total_power = 0
      site_ref = circuit.panel.site.site_ref

      live_result = db.execute("select * from emon_min_by_data where panel='#{site_ref}' and channel='CH-#{circuit.channel_no}' and asof_min>=#{Time.now.utc.beginning_of_day.to_i} and asof_min<=#{Time.now.utc.end_of_day.to_i} ALLOW FILTERING")
      ...
      redis.hmset("panel-#{site_ref}-CH-#{circuit.channel_no}", "max_power", value, "total_power", total_power)
    end
    totalPowerValue = 0
    ...
    redis.hmset("panel-#{site_ref}-totalPower", "max_power", value, "total_power", total_power)   
  end
