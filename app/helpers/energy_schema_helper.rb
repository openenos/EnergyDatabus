...
=begin
	...
	# Defaults to the system keyspace
		...
		session.execute("CREATE TABLE min_by_power_produced(site_ref text, asof_min int, value float, PRIMARY KEY(site_ref, asof_min)) WITH COMPACT STORAGE AND CLUSTERING ORDER BY(asof_min desc)")

		session.execute("CREATE TABLE hourly_power_produced(site_ref text, asof_hr int, value float, PRIMARY KEY(site_ref, asof_hr)) WITH COMPACT STORAGE AND CLUSTERING ORDER BY(asof_hr desc)")
